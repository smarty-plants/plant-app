...
export default function IndexPage() {
  ...
  const fetchNewData = async () => {
    const response = await fetch(process.env.API_URL + '/api/plants/').catch(
      (error) => {
    ...
    console.log(data);
    var i = 0;
    const plants = data.map(
      (plant: { plant_id: any; name: string; species: any }) => {
        i++;
        return {
          id: plant.plant_id,
          name: i + '. ' + plant.name,
  ...
  const handlePlantCreate = async () => {
    ...
    if (plantSpecies === '') {
      toast.error('Plant species cannot be empty', {
        position: 'bottom-right'
      });
      return;
    }
    if (temperatureMin === 0 && temperatureMax === 0) {
      toast.error('Temperature ranges cannot be empty', {
        position: 'bottom-right'
      });
      return;
    }
    if (humidityMin === 0 && humidityMax === 0) {
      toast.error('Humidity ranges cannot be empty', {
        position: 'bottom-right'
      });
      return;
    }
    if (sunlightMin === 0 && sunlightMax === 0) {
      toast.error('Sunlight ranges cannot be empty', {
        position: 'bottom-right'
      });
      return;
    }
    if (soilMoistureMin === 0 && soilMoistureMax === 0) {
      toast.error('Soil moisture ranges cannot be empty', {
        position: 'bottom-right'
      });
      return;
    }
    if (temperatureMin > temperatureMax) {
      toast.error('Min temperature cannot be greater than max temperature', {
        position: 'bottom-right'
      });
      return;
    }
    if (humidityMin > humidityMax) {
      toast.error('Min humidity cannot be greater than max humidity', {
        position: 'bottom-right'
      });
      return;
    }
    if (sunlightMin > sunlightMax) {
      toast.error('Min sunlight cannot be greater than max sunlight', {
        position: 'bottom-right'
      });
      return;
    }
    if (soilMoistureMin > soilMoistureMax) {
      toast.error(
        'Min soil moisture cannot be greater than max soil moisture',
        { position: 'bottom-right' }
      );
      return;
    }
    if (temperatureMin < -100 || temperatureMin > 100) {
      toast.error('Min temperature must be between -100 and 100', {
        position: 'bottom-right'
      });
      return;
    }
    if (temperatureMax < -100 || temperatureMax > 100) {
      toast.error('Max temperature must be between -100 and 100', {
        position: 'bottom-right'
      });
      return;
    }
    if (humidityMin < 0 || humidityMin > 100) {
      toast.error('Min humidity must be between 0 and 100', {
        position: 'bottom-right'
      });
      return;
    }
    if (humidityMax < 0 || humidityMax > 100) {
      toast.error('Max humidity must be between 0 and 100', {
        position: 'bottom-right'
      });
      return;
    }
    if (sunlightMin < 0 || sunlightMin > 100) {
      toast.error('Min sunlight must be between 0 and 100', {
        position: 'bottom-right'
      });
      return;
    }
    if (sunlightMax < 0 || sunlightMax > 100) {
      toast.error('Max sunlight must be between 0 and 100', {
        position: 'bottom-right'
      });
      return;
    }
    if (soilMoistureMin < 0 || soilMoistureMin > 100) {
      toast.error('Min soil moisture must be between 0 and 100', {
        position: 'bottom-right'
      });
      return;
    }
    if (soilMoistureMax < 0 || soilMoistureMax > 100) {
      toast.error('Max soil moisture must be between 0 and 100', {
        position: 'bottom-right'
      });
      return;
    }
    ...
    <main className="p-4 md:p-10 mx-auto max-w-7xl">
      <Dialog open={isOpen} onClose={(val) => setIsOpen(val)} static={true}>
        <DialogPanel>
          ...
          Your probe has been created successfully. You can now set id of your
          probe. Your probe id is:
      ...
      <Grid className="mt-6 gap-6">
        <Card>
          ...
          <TextInput
            onChange={handleProbeNameChange}
            placeholder="Probe name..."
          />
          <Text className="mt-2 mb-2">Plant</Text>
          <SearchSelect
            onValueChange={handleSelectChange}
            placeholder={isLoading ? 'Loading...' : 'Select plant...'}
          ...
          <Button
            ...
            disabled={
              isLoading || selectedPlant == '' || probeName == '' ? true : false
            }
          >
            Create
          </Button>
        ...
        <Card>
          ...
          <TextInput
            onChange={handlePlantSpeciesChange}
            placeholder="Plant specie..."
          />
          ...
          <Flex>
            <NumberInput
              ...
              max={100}
              onChange={(event) =>
                setTemperatureMin(parseInt(event.target.value))
              }
            ...
            <NumberInput
              onChange={(event) =>
                setTemperatureMax(parseInt(event.target.value))
              }
          ...
          <Flex>
            <NumberInput
              min={0}
              max={100}
              onChange={(event) => setHumidityMin(parseInt(event.target.value))}
              placeholder="Min humidity..."
            />
            <Text className="mx-2">-</Text>
            <NumberInput
              onChange={(event) => setHumidityMax(parseInt(event.target.value))}
          ...
          <Flex>
            <NumberInput
              min={0}
              max={100}
              onChange={(event) => setSunlightMin(parseInt(event.target.value))}
              placeholder="Min sunlight..."
            />
            <Text className="mx-2">-</Text>
            <NumberInput
              onChange={(event) => setSunlightMax(parseInt(event.target.value))}
          ...
          <Flex>
            <NumberInput
              ...
              max={100}
              onChange={(event) =>
                setSoilMoistureMin(parseInt(event.target.value))
              }
              placeholder="Min soil moisture..."
            />
            <Text className="mx-2">-</Text>
            <NumberInput
              onChange={(event) =>
                setSoilMoistureMax(parseInt(event.target.value))
              }
              placeholder="Max soil moisture..."
            />
          </Flex>
          <Button
            className="mt-4"
            onClick={handlePlantCreate}
            disabled={plantName == '' || plantSpecies == '' ? true : false}
          >
            Create
          </Button>
        </Card>
