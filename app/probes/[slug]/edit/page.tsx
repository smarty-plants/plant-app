...
export default function IndexPage({ params }: { params: { slug: string } }) {
  ...
  const fetchNewData = async () => {
    ...
    const response2 = await fetch(
      process.env.API_URL + '/api/probe/detail/' + params.slug + '/'
    ).catch((error) => {
    ...
    if (data2.plant) {
      ...
      console.log(plants);
      setSelectedPlant(
        plants.find((plant: { id: any }) => plant.id === data2.plant).name
      );
  ...
  const handleUpdateProbe = async () => {
    ...
    const foundPlant = plants.find(
      (plant: { name: string }) => plant.name === selectedPlant
    );
        ...
        <Grid className="mt-6 gap-6">
          <Card>
            ...
            <Flex>
              <Button
                className="mt-4"
                onClick={handleUpdateProbe}
                disabled={
                  isLoading || selectedPlant == '' || probeName == ''
                    ? true
                    : false
                }
              >
              ...
              <a href={'/probes/' + params.slug}>
                <Button className="mt-4 ml-4" variant="secondary" color="slate">
                  Back
                </Button>
              </a>
            </Flex>
