...
export default function IndexPage({ params }: { params: { slug: string } }) {
  ...
  const handleDateRangeChange: (value: DateRangePickerValue) => void = (
    value
  ) => {
  ...
  const fetchNewData = async () => {
    const response = await fetch(
      process.env.API_URL + '/api/probes/details/' + params.slug + '/'
    ).catch((error) => {
      ...
      {isLoading ? (
        ...
        <>
          <Flex>
            ...
            <Badge
              icon={data?.is_active ? SignalIcon : SignalSlashIcon}
              color={data?.is_active ? 'emerald' : 'red'}
            >
